...
如果你使用的是已发布到 registry 的镜像，请把 `agentlens:0.1.1` 替换为实际发布镜像名。Docker 适合快速体验和本地 viewer mode。高级 Agent Bridge 工作流建议使用 pipx 本机安装，因为 CLI/MCP 需要作为本机进程被 Claude Code、Codex、aider 或 Cursor 调用。
